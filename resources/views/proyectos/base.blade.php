{{-- resources/views/proyectos/base.blade.php --}}
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <title>{{ $pageTitle ?? 'Proyecto | NOVA' }}</title>
    ...
    <style>
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .hero{
            position:relative;
            height:100vh;
            background:url('{{ $heroImage ?? "https://images.pexels.com/photos/7031605/pexels-photo-7031605.jpeg?auto=compress&cs=tinysrgb&w=1600" }}') center/cover no-repeat;
            display:flex;
            align-items:center;
...
<section class="hero">
    <div class="hero-content">
        ...
        <p>{{ $heroText ?? 'Descripción del proyecto.' }}</p>
    </div>
</section>
...
<section class="detalle">
    <div class="detalle-texto">
        ...
        <div class="detalle-botones">
            @if(!empty($wazeUrl))
                <a href="{{ $wazeUrl }}" class="btn btn-light">📍 Llega con Waze</a>
            @endif
            @if(!empty($mapsUrl))
                <a href="{{ $mapsUrl }}" class="btn btn-light">🗺️ Llega con Maps</a>
            @endif
        </div>
        ...
        @if(!empty($areaFrom))
            <p><strong>Áreas desde:</strong> {{ $areaFrom }}</p>
        @endif
        @if(!empty($priceFrom))
            <p><strong>Precio desde:</strong> {{ $priceFrom }}</p>
        @endif

        <a href="{{ $ctaUrl ?? '#' }}" class="btn btn-brand">{{ $ctaText ?? 'AGENDAR VISITA' }}</a>
    </div>
